...
import type { ItemDetails, OrderData, PaymentMethod } from "../../globals/types/checkoutTypes"
...
<div className="grid sm:px-10 lg:grid-cols-2 lg:px-20 xl:px-32">
  <div className="px-4 pt-8">
    ...
    <div className="mt-8 space-y-3 rounded-lg border bg-white px-2 py-4 sm:px-6">
        ...
        <div key={items?.Product?.id} className="flex flex-col rounded-lg bg-white sm:flex-row">
        ...
        <div className="flex w-full flex-col px-4 py-4">
          <span className="font-semibold">{items?.Product?.productName}</span>
          <span className="float-right text-gray-400">Qty: {items?.Product?.productTotalStockQty}</span>
          <p className="text-lg font-bold">Rs. {items?.Product?.productPrice}</p>
        </div>
      </div>
                )
            })
    ...
    <form className="mt-5 grid gap-6">
      <div className="relative">
        <input className="peer hidden" id="radio_1" type="radio" name="radio" value="COD" onChange={handlePaymentMethod} />
      ...
      <div className="relative">
        <input className="peer hidden" id="radio_2" type="radio" value="khalti" onChange={handlePaymentMethod} name="radio"  />
        <span className="peer-checked:border-gray-700 absolute right-4 top-1/2 box-content block h-3 w-3 -translate-y-1/2 rounded-full border-8 border-gray-300 bg-white"></span>
    ...
    <div className="">
      ...
      <div className="flex flex-col sm:flex-row">
        <div className="relative flex-shrink-0 sm:w-7/12">
          <input type="text" id="billing-address" name="shippingAddress" className="w-full rounded-md border border-gray-200 px-4 py-3 pl-11 text-sm shadow-sm outline-none focus:z-10 focus:border-blue-500 focus:ring-blue-500"  onChange={handleChange} placeholder="Street Address"/>
